...
export async function GET(request: NextRequest) {
  try {
    ...
    const users: (User & { companyRole?: 'warehouse_owner' | 'warehouse_admin' | 'warehouse_staff' | null })[] = (profiles || []).map((profile: any) => {
      const company = Array.isArray(profile.companies) ? profile.companies[0] : profile.companies

      // Map profile role to company role: 'warehouse_owner' -> 'warehouse_owner', 'warehouse_admin' -> 'warehouse_admin', 'warehouse_staff' -> 'warehouse_staff'
      const companyRole: 'warehouse_owner' | 'warehouse_admin' | 'warehouse_staff' | null =
    ...
    const responseData: ListResponse<User & { companyRole?: 'warehouse_owner' | 'warehouse_admin' | 'warehouse_staff' | null }> = {
      success: true,
      data: users,
      total: users.length,
    }
