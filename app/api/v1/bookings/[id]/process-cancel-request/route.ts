...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (action === "approve") {
      // Cancel the booking (releases warehouse capacity)
      updatedBooking = await cancelBooking(bookingId);

      // Fetch raw payment fields not exposed on the Booking type
      const { data: rawBooking } = await supabase
        .from("bookings")
        .select("payment_intent_id, amount_paid, deposit_amount")
        .eq("id", bookingId)
        .single();

      const paymentIntentId: string | null = rawBooking?.payment_intent_id ?? null;
      const amountPaid: number = Number(rawBooking?.amount_paid ?? 0);
      const depositAmount: number = Number(
        rawBooking?.deposit_amount ?? booking.depositAmount ?? 0
      );

      // Issue time-based refund if deposit was paid
    ...
    const errorResponse = handleApiError(error, {
      path: `/api/v1/bookings/${(await params).id}/process-cancel-request`,
    });
    const errorData: ErrorResponse = {
      success: false,
