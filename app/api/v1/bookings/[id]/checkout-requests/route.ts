  ...
  try {
    ...
    if (palletIds.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: "No pallets found to check out",
          statusCode: 400,
        } satisfies ErrorResponse,
        { status: 400 }
      );
    ...
    // CONCURRENCY PROTECTION: Check if pallets are available and lock them atomically
    ...
    if (availError) {
      return NextResponse.json(
        {
          success: false,
          error: `Failed to check pallet availability: ${availError.message}`,
          statusCode: 500,
        } satisfies ErrorResponse,
        { status: 500 }
      );
    ...
    // Check if any pallets are unavailable
    const unavailable = (availabilityCheck || []).filter((item: any) => !item.can_checkout);
    if (unavailable.length > 0) {
      return NextResponse.json(
        {
          success: false,
          error: `Some pallets are unavailable for checkout`,
          statusCode: 409,
          details: unavailable,
        } satisfies ErrorResponse,
        { status: 409 }
    ...
    // Lock pallets atomically (prevents concurrent checkout)
    const { data: lockResult, error: lockError } = await supabase.rpc("lock_pallets_for_checkout", {
      p_inventory_item_ids: palletIds,
      p_locked_by: user.id,
    });
    ...
    if (lockError || !lockResult || lockResult.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: "Failed to lock pallets for checkout",
          statusCode: 500,
        } satisfies ErrorResponse,
        { status: 500 }
      );
    ...
    const { locked_count: _lockedCount, failed_items } = lockResult[0];
    ...
    if (failed_items && failed_items.length > 0) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: `Failed to lock ${failed_items.length} pallet(s)`,
          statusCode: 409,
          details: JSON.stringify({ failedItems: failed_items }),
        } satisfies ErrorResponse,
        { status: 409 }
      );
    }
    ...
    const amount = remainingTotal * (N / totalPalletCount) * (daysStored / totalBookingDays);
    ...
    const idempotencyKey = generateIdempotencyKey("checkout", bookingId, {
