...
async function runMigration() {
  try {
    ...
    const migrationPath = path.join(root, 'supabase', 'migrations', '100_add_warehouse_fees_and_transportation.sql');
    ...
    console.log('📄 Running migration: Add warehouse fees and transportation (100_add_warehouse_fees_and_transportation.sql)...\n');

    await client.query(migrationSQL);
    console.log('✅ Migration 100 applied successfully!');
    console.log('   Added warehouse_in_fee, warehouse_out_fee, need_transportation, container_40_dc, container_40_hc, container_20_dc columns.');
    ...
    if (error.code === '42701') {
      console.log('\n   Note: Some columns may already exist. This is normal if migration was partially applied.');
    }
    await client.end();
    process.exit(1);
  }
