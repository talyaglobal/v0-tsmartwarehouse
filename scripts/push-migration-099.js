...
async function runMigration() {
  try {
    ...
    const migrationPath = path.join(root, 'supabase', 'migrations', '099_make_old_columns_nullable_in_warehouse_services.sql');
    ...
    console.log('📄 Running migration: Make old columns nullable in warehouse_services (099_make_old_columns_nullable_in_warehouse_services.sql)...\n');

    await client.query(migrationSQL);
    console.log('✅ Migration 099 applied successfully!');
    console.log('   Old columns (name, category, unit_type) are now nullable in warehouse_services table.');
