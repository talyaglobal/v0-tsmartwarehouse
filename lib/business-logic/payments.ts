...
export async function processInvoicePayment(
    ...
    if (booking) {
      if (booking.status === "pre_order") {
        if (!booking.scheduledDropoffDatetime) {
          throw new Error(
            "Time slot has not been set by warehouse worker. Please wait for warehouse to assign a time slot."
          );
        }
        if (!booking.timeSlotConfirmedAt) {
          throw new Error(
            "Time slot has not been confirmed. Please confirm the assigned time slot before proceeding with payment."
          );
        }
      }
    }
  ...
  if (
    input.useCreditBalance ||
    input.paymentMethod === "credit_balance" ||
    input.paymentMethod === "both"
  ) {
  ...
  if (remainingAmount > 0 && (input.paymentMethod === "card" || input.paymentMethod === "both")) {
    ...
    if (!input.paymentMethodId) {
      ...
      if (invoice.bookingId) {
        ...
        try {
          const {
            data: { user: authUser },
            error: authError,
          } = await supabase.auth.admin.getUserById(input.customerId);
          ...
          throw new Error(
            "Customer email not found. Please ensure the booking exists and has customer information."
          );
        }
      }
      ...
      if (!customerEmail) {
        throw new Error(
          "Customer email is required for payment processing. Please contact support."
        );
      }
...
export async function confirmPayment(
  ...
  const paymentIntent = await confirmPaymentIntent(payment.stripePaymentIntentId, paymentMethodId);
...
/**
 * Calculate the refund amount for a cancelled booking based on the
 * time-based refund policy:
 *   >48h before start  → 100% of deposit
 *   24–48h before start → 50% of deposit
 *   <24h before start   → 0%
 *   active/completed    → 0%
 *
 * @param depositAmount - The amount actually paid as deposit (not total amount)
 * @param startDate - Booking start date
 * @param bookingStatus - Current booking status
 * @param cancelledAt - When the cancellation was requested (defaults to now)
 */
export function calculateCancellationRefund(
  ...
  const hoursUntilStart = (start.getTime() - cancelledAt.getTime()) / (1000 * 60 * 60);
  ...
  if (hoursUntilStart > 24) {
    return {
      refundAmount: Math.round(depositAmount * 0.5 * 100) / 100,
      refundPercent: 50,
...
/**
 * Process a booking cancellation with Stripe refund.
 * Calculates refund per policy, issues Stripe refund if deposit was paid,
 * and records the result in the bookings row.
 *
 * Guards:
 *   - refundAmount is capped at depositAmount (never exceeds paid amount)
 *   - Skips Stripe call if deposit was never paid
 */
export async function processCancellationRefund(
  ...
  const { refundAmount, refundPercent, reason } = calculateCancellationRefund(
    depositAmount,
    startDate,
    bookingStatus
  );
  ...
  if (safeRefundAmount > 0 && paymentIntentId) {
    try {
      const { refundPaymentIntent } = await import("@/lib/payments/stripe");
      ...
      const stripeRefund = await refundPaymentIntent({
        ...
        idempotencyKey,
        metadata: { booking_id: bookingId, refund_reason: reason },
      });
      stripeRefundId = stripeRefund?.id ?? null;
    } catch (err) {
      refundError = err instanceof Error ? err.message : "Stripe refund failed";
      console.error("Cancellation Stripe refund failed:", refundError);
    }
  }

  // Persist refund details to booking row regardless of Stripe outcome
  await supabase
    .from("bookings")
    .update({
  ...
  if (safeRefundAmount > 0) {
    await supabase.rpc("log_payment_event", {
      ...
      p_event_type: refundError ? "refund_failed" : "refund_succeeded",
      p_amount: safeRefundAmount,
      p_stripe_refund_id: stripeRefundId,
      p_status: refundError ? "failed" : "completed",
      p_error_message: refundError,
      p_metadata: { refund_percent: refundPercent, reason },
    });
  }

  return { refundAmount: safeRefundAmount, refundPercent, stripeRefundId, error: refundError };
}
