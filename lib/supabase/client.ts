...
// Kolaybase integration
const USE_KOLAYBASE =
  process.env.NEXT_PUBLIC_USE_KOLAYBASE === "true" ||
  process.env.NEXT_PUBLIC_KOLAYBASE_ANON_KEY !== undefined;
...
export function createClient() {
  // Check if Kolaybase should be used
  if (USE_KOLAYBASE && process.env.NEXT_PUBLIC_KOLAYBASE_ANON_KEY) {
    // Import and return Kolaybase client
    const { createClient: createKolaybaseClient } = require("../kolaybase/client");
    return createKolaybaseClient();
  }
